...
### Fixed
- **Factual doc + unambiguous script bugs** — README/README_zh/RELEASE-CHECKLIST now match routing.json (44 routes, R45 present, 175 benchmark cases) and generic bootstrap-manifest (26 capabilities); RULES/RULES_zh/README/INDEX/plugin adapter use platform-native `master-route.ps1|.sh`; `extract-summaries.ps1` INDEX tree emits module directories (not `SKILL.md/` paths); `case-init.sh` default `auth.basis` is `own_system` (contract-allowed, PS parity); `auto-merge-journal.yml` failure comment uses `failure()`; generic `refresh-tool-index.sh` catalogs `bkcrack`; ToolDiscovery FixedVersion placeholders replaced with manifest pins or cleared; PLATFORMS.md notes Kali-only capability names. (pwntools verifyCommand left for #152 — scripts treat it as a binary name today.) Related discussion: #149–#154 (design-sensitive items intentionally not claimed as closed here).
